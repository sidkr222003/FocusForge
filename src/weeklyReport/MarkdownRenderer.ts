...
export class MarkdownRenderer {
  ...
  static renderIssues(entries: WeeklyReport['issuesWorkedOn']): string {
    if (entries.length === 0) {
    ...
    for (const entry of entries) {
      const { issue, repoSlug, sessionsCount, totalMinutes, finalStatus, outcomeNote, completedAt, filesTouched, changeSummary } =
        entry;
